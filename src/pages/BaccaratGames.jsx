...
import { Button, Link, Image, Chip, Modal, ModalContent, ModalBody, ModalHeader, ModalFooter, useDisclosure, Card, CardHeader, CardBody, CardFooter } from '@nextui-org/react';
...
const BaccaratGames = () => {
    ...
    const fetchGameTypeInfo = async () => {
        try {
            const data = await GameService.getGameType(4);
    ...
    const fetchBaccaratGames = async () => {
        try {
            const data = await GameService.getGamesByType(4); // 4 -> tipo Baccarat
            setGames(data);
        } catch (error) {
            toast.error(error.message || 'Error desconocido');
        }
